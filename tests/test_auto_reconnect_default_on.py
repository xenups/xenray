"""Auto-reconnect default-ON behaviour tests.

Locks in the contract that auto-reconnect is ENABLED BY DEFAULT:

1. SettingsRepository.get_auto_reconnect_enabled() returns True when the
   settings file is absent (default-on) and False after an explicit disable.
2. ConnectionMonitoringService honors the setting: when auto-reconnect is
   disabled, monitoring does not start, so failures never reach the
   reconnect path.
"""
...
class TestAutoReconnectDefaultOn:
    """SettingsRepository: auto-reconnect defaults to ON."""

    def test_auto_reconnect_defaults_to_true_when_file_absent(self, tmp_path):
...
class TestMonitoringServiceHonorsAutoReconnect:
    ...
    def test_monitoring_starts_when_enabled_by_default(self, tmp_path):
        repo = SettingsRepository(str(tmp_path))  # file absent -> default on
        app_context = Mock()
        app_context.settings = repo
        app_context.load_config = Mock(return_value=({}, None))
