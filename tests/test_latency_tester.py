...
from src.services.connection.connection_tester import GENERATE_204_URLS, ConnectionTester
...
def test_probe_204_uses_gstatic_and_streams_headers_only():
    with patch("src.services.connection.connection_tester.requests.get", return_value=_fake_resp(204)) as mock_get:
    ...
    assert mock_get.call_args.kwargs["timeout"] == 5.0  # hard per-node timeout (SNI-injector friendly)


def test_probe_204_fails_over_to_cloudflare_on_timeout():
    resp = _fake_resp(204)
    with patch(
        "src.services.connection.connection_tester.requests.get",
...
def test_batch_tests_all_profiles_concurrently():
    ...
    with patch(
        "src.services.connection.latency_tester.ConnectionTester.test_connection_sync",
