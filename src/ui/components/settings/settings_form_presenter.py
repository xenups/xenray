...
class SettingsFormPresenter:
    ...
    def save_tun_engine(self, engine_name: Any):
        """Save selected TUN engine."""
        engine = self._extract_str_value(engine_name)
        self._app_context.settings.set_tun_engine(engine)
        self._show_toast(t("settings.tun_engine_saved"), "success")
