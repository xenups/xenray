...
class DNSPage(ft.Container):
    ...
    def _refresh_list(self, update=True):
        ...
        for idx, item in enumerate(self._dns_list):
            ...
            row = ft.Container(
                content=ft.Row([
                    ft.Container(
                        content=ft.Text(proto, size=10, weight=ft.FontWeight.BOLD, color=ft.Colors.ON_SURFACE if proto in ["UDP", "TCP"] else ft.Colors.ON_PRIMARY_CONTAINER),
    ...
    def _add_server(self, e):
        ...
        entry = {
            "address": addr,
            "protocol": self._protocol_dd.value,
            "domains": []
        }
    ...
    def _move_up(self, idx):
        if idx > 0:
            self._dns_list[idx], self._dns_list[idx-1] = self._dns_list[idx-1], self._dns_list[idx]
            self._save()
            self._refresh_list()
