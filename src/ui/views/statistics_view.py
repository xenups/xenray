...
class StatisticsView(ft.Container):
    ...
    def __init__(self, on_back_click: Callable | None = None):
        ...
        self._traffic_rate_badge = ft.Container(
            content=ft.Row(
                [
                    ...
                    ft.Text("0.0 MB/s", size=13, weight=ft.FontWeight.W_800, color=WHITE),
                ],
                spacing=6,
        ...
        self._dl_speed_text = ft.Text("0.0 MB/s", size=18, weight=ft.FontWeight.W_800, color=WHITE)
        self._ul_speed_text = ft.Text("0.0 MB/s", size=18, weight=ft.FontWeight.W_800, color=WHITE)
        self._dl_total_text = ft.Text("0.0 MB", size=16, weight=ft.FontWeight.W_700, color=WHITE)
        self._ul_total_text = ft.Text("0.0 MB", size=16, weight=ft.FontWeight.W_700, color=WHITE)
        self._total_transfer_text = ft.Text("0.0 MB", size=18, weight=ft.FontWeight.W_800, color=CYAN)
        self._uptime_display_text = ft.Text("00:00:00", size=15, weight=ft.FontWeight.W_700, color=WHITE)
        self._peak_speed_text = ft.Text("0.0 MB/s", size=15, weight=ft.FontWeight.W_700, color=WHITE)
            ...
            [
                ft.Row(
                    [
                        ...
                        ft.Text("REAL-TIME TRAFFIC WAVE STREAM", size=11, weight=ft.FontWeight.W_700, color=MUTED_WHITE),
                    ],
                    spacing=6,
                ),
                ft.Row(
                    [
                        ...
                        ft.Text("Download", size=11, color=MUTED_WHITE, weight=ft.FontWeight.W_600),
                        ft.Container(width=12),
                        ft.Container(width=8, height=8, border_radius=4, bgcolor="#38bdf8"),
                        ft.Text("Upload", size=11, color=MUTED_WHITE, weight=ft.FontWeight.W_600),
                    ],
                    spacing=4,
                ),
            ],
        ...
        visualizer_box = create_glass_container(
            content=ft.Column(
                [
                    wave_header,
                    ft.Container(content=chart_row, height=140, padding=ft.Padding.symmetric(vertical=6)),
                ],
                spacing=8,
            ),
            expand=True,
            ...
            [
                ft.Row(
                    [
                        ...
                        ft.Text(title, size=11, weight=ft.FontWeight.W_700, color=AppColors.ON_SURFACE_VARIANT),
                    ],
                    spacing=8,
                ),
                val_control,
    ...
    def update_network_stats(
        ...
        # Calculate combined total transfer
        try:
            ...
            if cur_max > self._peak_bps:
                ...
                self._peak_speed_text.value = f"{peak_kb:.1f} KB/s" if peak_kb < 1024.0 else f"{(peak_kb/1024.0):.1f} MB/s"
        ...
        dl_heights = self._compute_smooth_wave_heights(self._dl_history, num_output=32, min_h=6.0, max_h=140.0)
        ul_heights = self._compute_smooth_wave_heights(self._ul_history, num_output=32, min_h=6.0, max_h=140.0)
