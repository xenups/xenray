...
from src.core.constants import (APPDIR, TEMP_ROOT, TUN2PROXY_EXECUTABLE,
                                TUN_LOG_FILE)
...
class Tun2ProxyService:
    ...
    def _start_windows(self, bin_path: str, socks_port: int, bypass_ips: List[str]) -> Optional[int]:
        ...
        cmd = [
            ...
            "--dns", "virtual",
            "-v", "trace"  # Increased verbosity for debugging
        ]
        ...
        self._process = ProcessUtils.run_command(
            cmd,
            stdout_file=TUN_LOG_FILE,
            stderr_file=TUN_LOG_FILE
        )
    ...
    def _stop_linux(self, pid: int) -> bool:
        ...
        cmd = [
            "pkexec",
            os.path.join(TEMP_ROOT, "run_vpn.sh"),
            "stop",
            str(pid)
        ]
...
"""
        ...
        os.makedirs(os.path.dirname(os.path.join(TEMP_ROOT, "run_vpn.sh")), exist_ok=True)
