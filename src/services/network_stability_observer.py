...
class NetworkStabilityObserver:
    ...
    def _update_handshake_metrics(self):
        ...
        recent_samples = [
            duration for ts, duration in self._handshake_history
    ...
    def _check_overrides(self, metrics: dict) -> tuple[Optional[NetworkQuality], bool, Optional[str]]:
        ...
        high_confidence_types = {
            ErrorType.DNS_FAILURE, ErrorType.TIMEOUT, 
            ErrorType.TLS_FAILURE, ErrorType.PROCESS_CRASH
        }
        ...
        if now - self._startup_time < AdaptationConfig.SILENCE_WARMUP:
            ...
            (now - self._last_error_time) > AdaptationConfig.SILENCE_TIMEOUT):
            return (
                NetworkQuality.CRITICAL, 
                True, 
