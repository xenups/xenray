"""TLS ClientHello construction/parsing — ported from patterniha/SNI-Spoofing
``utils/packet_templates.py`` (ClientHelloMaker).

Builds the fixed 517-byte TLS 1.3 ClientHello carrying only the SNI + padding +
key-share extensions. The static template bytes are taken verbatim from the
reference repository (MIT-licensed) so wire output stays byte-identical.
"""
...
class ClientHelloMaker:
    ...
    def parse_client_hello(cls, client_hello_bytes: bytes):
        ...
        tls_sni = client_hello_bytes[127 : 127 + (struct.unpack("!H", client_hello_bytes[125:127])[0])]
