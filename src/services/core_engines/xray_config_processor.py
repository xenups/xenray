...
class XrayConfigProcessor:
    ...
    @staticmethod
    def save_config(config: dict, path: str = OUTPUT_CONFIG_PATH) -> bool:
        """Serialize and persist the processed Xray JSON configuration to disk."""
        try:
            os.makedirs(os.path.dirname(os.path.abspath(path)), exist_ok=True)
            with open(path, "w", encoding="utf-8") as f:
                json.dump(config, f, indent=2)
            logger.debug(f"[XrayConfigProcessor] Config saved successfully to {path}")
            return True
        except Exception as e:
            logger.error(f"[XrayConfigProcessor] Failed to save Xray config to {path}: {e}")
            return False
