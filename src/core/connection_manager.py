...
from src.core.constants import (OUTPUT_CONFIG_PATH,
                                ...
                                RATE_LIMIT_AIMD_ADDITIVE_INCREASE,
                                RATE_LIMIT_AIMD_MULTIPLICATIVE_DECREASE,
                                RATE_LIMIT_DEFAULT_BURST,
                                RATE_LIMIT_DEFAULT_RATE, RATE_LIMIT_ENABLED,
                                RATE_LIMIT_MAX_RATE, RATE_LIMIT_MIN_RATE)
...
class ConnectionManager:
    ...
    def __init__(self, config_manager: ConfigManager):
        ...
        # Initialize rate limiter for VPN mode (only if enabled)
        self._rate_limiter: Optional[RateLimiterService] = None
        if RATE_LIMIT_ENABLED:
            self._rate_limiter = RateLimiterService(
                initial_rate=RATE_LIMIT_DEFAULT_RATE,
                initial_burst=RATE_LIMIT_DEFAULT_BURST,
                min_rate=RATE_LIMIT_MIN_RATE,
                max_rate=RATE_LIMIT_MAX_RATE,
                adaptation_enabled=RATE_LIMIT_ADAPTATION_ENABLED,
                additive_increase=RATE_LIMIT_AIMD_ADDITIVE_INCREASE,
                multiplicative_decrease=RATE_LIMIT_AIMD_MULTIPLICATIVE_DECREASE,
            )
            logger.info("[ConnectionManager] Rate limiter initialized for VPN mode")
        
        # Initialize passive log monitor if observer is available
        self._log_monitor: Optional[PassiveLogMonitor] = None
        if self._rate_limiter and self._rate_limiter.observer:
            self._log_monitor = PassiveLogMonitor(self._rate_limiter.observer)
    ...
    def _check_rate_limit(self, mode: str, step_callback) -> bool:
        ...
        result = loop.run_until_complete(
            self._rate_limiter._throttle.limit("vpn_connection", cost=1)
        )
        
        if result.limited:
            retry_after = result.retry_after or 1.0
            logger.warning(
                f"[ConnectionManager] Connection rate limited. Retry after {retry_after:.1f}s"
            )
            if step_callback:
