...
class SubscriptionManager:
    ...
    def _parse_subscription_content(self, content: str) -> List[dict]:
        ...
        try:
            ...
            logger.warning(f"JSON parsing failed: {e.msg} at line {e.lineno} col {e.colno}")
        ...
        if not content.startswith("vless://") and not content.startswith("vmess://") and "outbounds" not in content:
            try:
        ...
        for link in links:
            ...
            try:
                ...
                logger.warning(f"Skipping invalid link in sub: {link[:20]}... Error: {e}")
                continue
